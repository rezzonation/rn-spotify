...
public class RNSpotifyModule extends ReactContextBaseJavaModule implements Player.NotificationCallback, ConnectionStateCallback, RNEventConformer {
  ...
  private Player.OperationCallback mOperationCallback = new Player.OperationCallback() {
    ...
    public void onSuccess() {
        Log.e("SpotifyPlayer","OK!");
    }

    @Override
    public void onError(Error error) {
       Log.e("SpotifyPlayer","ERROR:" + error);
    }
  };
  ...
  RNSpotifyModule(ReactApplicationContext reactContext) {
    ...
    AuthActivity.spotifyModule = this;
  ...
  private void createChannel() {
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
      NotificationChannel channel = new NotificationChannel(ForeGroundPlayerServiceSpotify.CHANNEL_ID,
        "RezzonationSpotify", NotificationManager.IMPORTANCE_DEFAULT);
  ...
  BroadcastReceiver broadcastReceiverSpotify = new BroadcastReceiver() {
    @Override
    public void onReceive(Context context, Intent intent) {
      ...
      switch (action) {
        case ForeGroundPlayerServiceSpotify.ACTION_PREVIUOS:
          ...
          dataEmitToJS();
          break;
        case ForeGroundPlayerServiceSpotify.ACTION_PLAY:
  ...
  public void dataEmitToJS(){
    this.sendEvent("previousTrack",createPlaybackEvent());
  }
  ...
  @ReactMethod
  public void clearSpotifyNotification() {
    clearNotification();
  }
  ...
  public void onPlaybackEvent(PlayerEvent playerEvent) {
    switch (playerEvent) {
      ...
      case kSpPlaybackNotifyTrackChanged:
        ...
        Log.e("Player_trackChange", "-----trackChange------");

        if (paused == true ){
          mMetadata = player.getMetadata();
          startService(mMetadata.currentTrack.name, mMetadata.currentTrack.artistName, mMetadata.currentTrack.albumCoverWebUrl, R.drawable.ic_play_arrow_black_24dp);
        }
        Log.e("Player_trackChnage", mMetadata.toString());
        break;
  ...
  public void startService(String trackName, String artistName, String trackImageUrl, int play_button) {
    Intent serviceIntent = new Intent(reactContext, ForeGroundPlayerServiceSpotify.class);
    serviceIntent.putExtra("trackName", trackName);
    serviceIntent.putExtra("artistName", artistName);
    serviceIntent.putExtra("trackImageUrl", trackImageUrl);
    serviceIntent.putExtra("playbutton", play_button);
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
      Log.d("BackgroundService", "startForegroundService");
